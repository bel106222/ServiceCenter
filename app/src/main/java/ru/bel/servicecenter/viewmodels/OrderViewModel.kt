    ...
    fun saveOrder() {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                // ============================================================
                // ШАГ 1. Если это НОВЫЙ заказ — создаём его с is_completed = false,
                // чтобы вебхук на "заказ завершён" не сработал раньше времени.
                // ============================================================
                if (isNewOrder) {
                    val draftOrder = order.copy(is_completed = false)
                    RepositoryProvider.orderRepo.createOrder(draftOrder)
                }

                // ============================================================
                // ШАГ 2. Удаляем позиции, которых больше нет
                // ============================================================
                ...
                // ============================================================
                // ШАГ 4. Теперь, когда позиции в БД, обновляем заказ
                // (или досоздаём его для нового заказа — с настоящим is_completed).
                // Именно здесь сработает вебхук "заказ завершён" — и позиции уже на месте.
                // ============================================================
                if (isNewOrder) {
                    // Для нового заказа: обновляем, только если нужно установить завершённость.
                    // Если заказ не завершён — оставляем как есть (уже создан на шаге 1).
                ...
                // ============================================================
                // ШАГ 5. Вложения: удаляем те, что убрал пользователь
                // ============================================================
                ...
                // ============================================================
                // ШАГ 6. Вложения: загружаем новые
                // ============================================================
                ...
                // ============================================================
                // ШАГ 7. Сигналим успех. Кэш обновляем в фоне.
                // ============================================================
