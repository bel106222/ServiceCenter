 ...
 * Позволяет редактировать личные данные и видеть привязанного клиента.
...
fun UserProfileScreen(
    ...
    // Название привязанного клиента
    var clientTitle by remember { mutableStateOf<String?>(null) }

    // Загрузка названия клиента по client_id
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Text("Мой профиль", style = MaterialTheme.typography.headlineSmall)
        Spacer(modifier = Modifier.height(8.dp))

        // Отображение привязанного клиента
        if (clientTitle != null) {
        ...
        Button(
            onClick = {
                ...
                if (listOf(nameError, emailError, phoneError, passwordError).all { it == null }) {
                    ...
                    // client_id не меняем (пользователь может сменить только через специальный экран)
